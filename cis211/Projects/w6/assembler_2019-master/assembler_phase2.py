...
def assemble(lines: List[str]) -> List[int]:
    ...
    What this function does:
        takes a list of source lines
        returns a list of Instruction objects
        translate instructions like the two positive examples

    What I Need To Do For Phase 1:
        convert lines like the two negative examples into lines like the two positive examples
    """

    error_count = 0 # ct for counting likes that we can't translate
    instructions = [ ]
    for lnum in range(len(lines)):
        ...
        try: 
            ...
            if fields["kind"] == AsmSrcKind.FULL:
                ...
                word = instr.encode()
                instructions.append(word) # Appending Instruction objects to the instruction list which will be returned
...
def cli() -> object:
    ...
    sourcefile: sys.stdin (line 306)
    How this works:
        whole file is read into a list with readlines, and then the assemble
        function converts that to a list of Instruction objects, which are printed
        in a loop at the end of the function

    For Assembler Phase 1:
        read all the lines into a list and then process them as main does
    """
