...
Project Page:
    https://classes.cs.uoregon.edu/20S/cis211/projects/cpu.php

Author: Ryan Heise
"""
# We need the BitField class from last week to extract the parts of an instruction word
from bitfield import BitField
# For the condition codes, a special kind of enum called a Flag is useful
...
# names:
#   r0 is called 'zero' because on the DM2020W it always
#   holds value 0,
#
#   and r15 is called 'pc' because it is used to
...
class CondFlag(Flag):
    ...
    represent "non-negative result" as CondFlag.Z | CondFlag.P
    CondFlag.NEVER is a CondFlag object in which all of the bits are zero
    CondFlag.ALWAYS is a CondFlag object in which all of the bits are one
    """
    M = 1  # Minus (negative) (low-order bit)
    Z = 2  # Zero (bit of 1)
    P = 4  # Positive (bit of 2)
    V = 8  # Overflow (arithmetic error, e.g., divide by zero) (bit of 3)
    NEVER = 0
    ALWAYS = M | Z | P | V
...
def decode(word: int) -> Instruction:
    ...
    ext_instr = instr_field.extract(word)
    ext_cond = cond_field.extract(word)
    ext_reg = reg_target_field.extract(word)
    ext_src1 = reg_src1_field.extract(word)
    ext_src2 = reg_src2_field.extract(word)
    ext_offset = offset_field.extract_signed(word)
    return Instruction(OpCode(ext_instr), CondFlag(ext_cond), ext_reg, ext_src1, ext_src2, ext_offset)
