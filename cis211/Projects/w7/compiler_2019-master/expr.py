...
class Read(Expr):
    ...
    def gen(self, context: Context, target: str):
        """read value into target reg by using LOAD from 510"""
...
class If(Control):
    ...
    def gen(self, context: Context, target: str):
        # checks, labels, and jumps present in the While code generation
        e = context.new_label("else")
        fi = context.new_label("fi")
        self.cond.condjump(context, target, e, jump_cond=False)
        self.thenpart.gen(context, target)
        context.add_line(f"    JUMP {fi}")
        context.add_line(f"{e}:")
        self.elsepart.gen(context, target)
        context.add_line(f"{fi}:")