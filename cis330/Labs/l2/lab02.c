...
{ 
  ...
  printf("\nPart 3: Updated Matrix:\n");
  // Capitalize first col
  apply_by_row(capitalize_first_col, data, data_rows, data_cols);
  // Print out updated matrix
  apply_by_row(print_str, data, data_rows, data_cols);

  printf("\nPart 5: Updated Matrix (apply_by_col)\n");
...
{
  // printf("String: %s \tRow: %d \tCol: %d \tRows: %d \tCols: %d\n", &in_str[0], row, col, rows, cols);
  int i;
  // If string is in the first col
  if(col == 0){
    // Capitalize the first letter in the string
    for(i = 0; i < sizeof(in_str); i++){
